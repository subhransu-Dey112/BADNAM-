...
class SetupConfigView(discord.ui.View):
    ...
    async def update_preview(self, interaction):
        ...
        if cfg["embed_image"].startswith("http"): preview_embed.set_image(url=cfg["embed_image"])
    ...
    async def confirm_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if cfg["embed_image"].startswith("http"): final_embed.set_image(url=cfg["embed_image"])
...
class Recovery(commands.Cog):
    def __init__(self, bot):
        ...
        self._load_dbs()
        self.instant_role_loop.start()

    def cog_unload(self):
        self.instant_role_loop.cancel()
    ...
    # ==========================================
    # ⚡ INSTANT MEMORY-QUEUE ROLE ASSIGNER
    # ==========================================
    @tasks.loop(seconds=2)
    async def instant_role_loop(self):
        if not shared.pending_users: return
        
        # Copy queue and clear it to manage processing load
        users_to_process = list(shared.pending_users)
        shared.pending_users.clear()
    ...
    @instant_role_loop.before_loop
    async def before_instant_role(self):
    ...
    async def auth_setup(self, ctx):
        ...
        await ctx.send(embeds=[preview_embed, config_embed], view=SetupConfigView(self, ctx, target_channel))
    ...
    async def pull_members(self, ctx, amount: int = None):
        if not self.is_allowed(ctx): return
        if self.pulling_active: return await ctx.send("❌ A recovery pull operation is already running!")
        
        cfg = self.get_guild_config(ctx.guild.id)
        role_id = cfg["verify_role"]
        if not role_id: return await ctx.send("❌ Configure your verification role first via `b!auth role`.")

        CLIENT_ID = os.environ.get("CLIENT_ID")
        CLIENT_SECRET = os.environ.get("CLIENT_SECRET")
        if not CLIENT_ID or not CLIENT_SECRET: return await ctx.send("❌ Setup Error: Client credentials missing on Render environment settings.")

        self.pulling_active = True
        self._load_dbs() 
        msg = await ctx.send(f"🔄 **Starting Member Recovery...** Scanning database.")
        success, failed_api, failed_refresh, skipped_present = 0, 0, 0, 0
            ...
            for user_id, keys in token_list:
                ...
                async with session.put(url, headers=headers, json=body) as resp:
                    if resp.status in [201, 204]:
                        ...
                        refresh_data = {'client_id': CLIENT_ID, 'client_secret': CLIENT_SECRET, 'grant_type': 'refresh_token', 'refresh_token': keys["refresh_token"]}
                        async with session.post("https://discord.com/api/v10/oauth2/token", data=refresh_data, headers={'Content-Type': 'application/x-www-form-urlencoded'}) as refresh_resp:
                            if refresh_resp.status == 200:
                                ...
                                async with session.put(url, headers=headers, json={"access_token": new_tokens["access_token"]}) as final_resp:
    ...
    async def snapshot_create(self, ctx):
        ...
        await ctx.send("✅ **Server Snapshot Saved!**")

    @snapshot_group.command(name="channels")
    async def restore_channels(self, ctx):
        if not self.is_allowed(ctx): return
        gid = str(ctx.guild.id)
        if gid not in self.snapshots or "channels" not in self.snapshots[gid]: return await ctx.send("❌ No snapshot found.")
        await ctx.send("🛠️ **Rebuilding Server Channels...**")
        categories = {}
        for chan in sorted(self.snapshots[gid]["channels"], key=lambda x: x["position"]):
            if chan["type"] == "category": categories[chan["name"]] = await ctx.guild.create_category(name=chan["name"])
        for chan in sorted(self.snapshots[gid]["channels"], key=lambda x: x["position"]):
        ...
        await ctx.send("✅ Channels fully restored.")

    @snapshot_group.command(name="roles")
    async def restore_roles(self, ctx):
        if not self.is_allowed(ctx): return
        gid = str(ctx.guild.id)
        if gid not in self.snapshots or "roles" not in self.snapshots[gid]: return await ctx.send("❌ No snapshot found.")
